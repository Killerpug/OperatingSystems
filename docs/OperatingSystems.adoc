...
The kernel must allocate and free physical memory at run-time for: user memory, kernel stacks, pipe buffers, mainly...
...
|int fork()| Create a process by duplicating the parent process(file descriptor table along with its memory). They run in separate memory spaces. Return values: For the child process returns 0. For the parent returns the PID of the child process, which is a positive number. On failure -1 is returned to the parent.
